...
// Table 6. INVSBOX()
const unsigned char inv_subtitution_box[16][16] = {
...
void aes_inv_subbyte(unsigned char* byte) {
    unsigned char col = *byte >> 4;
    unsigned char row = *byte & 0x0F;
    *byte             = inv_subtitution_box[col][row];
}

void aes_inv_subbytes(unsigned char* state) {
    for (int i = 0; i < STATE_SIZE; i++)
        aes_inv_subbyte(state + i);
}
...
void aes_cipher(char* input, struct aes_context* ctx) {
    ...
    for (int round = 1; round < ctx->key_rounds; round++) {
        ...
        aes_add_round_key(ctx->state->data,
                          (ctx->key_schedule->data + (round * STATE_SIZE)));
    }
    ...
    aes_shiftrows(ctx->state->data);
    aes_add_round_key(ctx->state->data,
                      ctx->key_schedule->data + (ctx->key_rounds * STATE_SIZE));
}

void aes_inv_cipher(char* input, struct aes_context* ctx) {
    // 1 input -> state
    memcpy(ctx->state->data, input, STATE_SIZE);
    ...
    for (int round = (ctx->key_rounds - 1); round > 0; round--) {
        aes_inv_shiftrows(ctx->state->data);
        aes_inv_subbytes(ctx->state->data);
        aes_add_round_key(ctx->state->data,
                          (ctx->key_schedule->data + (round * STATE_SIZE)));
        aes_inv_mix_columns(ctx->state->data);
    }
...
void aes_inv_mix_columns(unsigned char* state) {
    for (int i = 0; i < 4; i++) {
        ...
        char r0 = aes_x_times(*a0, 0x0E) ^ aes_x_times(*a1, 0x0B) ^
                  aes_x_times(*a2, 0x0D) ^ aes_x_times(*a3, 0x09);
        char r1 = aes_x_times(*a0, 0x09) ^ aes_x_times(*a1, 0x0E) ^
                  aes_x_times(*a2, 0x0B) ^ aes_x_times(*a3, 0x0D);
        char r2 = aes_x_times(*a0, 0x0D) ^ aes_x_times(*a1, 0x09) ^
                  aes_x_times(*a2, 0x0E) ^ aes_x_times(*a3, 0x0B);
        char r3 = aes_x_times(*a0, 0x0B) ^ aes_x_times(*a1, 0x0D) ^
                  aes_x_times(*a2, 0x09) ^ aes_x_times(*a3, 0x0E);
