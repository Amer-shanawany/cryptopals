#include "bytes.h"
#include <CUnit/CUnit.h>
#include <CUnit/CUnitCI.h>
#include <aes.h>

#include <aes.h>

void test_aes_key_expansion_AES_128() {
    // 2b7e151628aed2a6abf7158809cf4f3c
    unsigned char key[] = {
        0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
        0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c,
    };
    unsigned char expanded_key[] = {
        ,
    };

    struct aes_context* ctx = aes_init_context(AES_128);
    CU_ASSERT_PTR_NOT_NULL_FATAL(ctx);

    for (int i = 0; i < 16; i++) {
        ctx->key->data[i] = key[i];
    }

    int result = aes_key_expansion(ctx);
    CU_ASSERT_EQUAL(result, 0);

    for (int i = 0; i < ctx->key_schedule->length; i++) {
        CU_ASSERT_EQUAL(expanded_key[i], ctx->key_schedule->data[i]);
    }

    aes_free_context(&ctx);
    CU_ASSERT_PTR_NULL(ctx);
}

void test_aes_key_expansion_AES_192() {
    unsigned char key[] = {
        0x8E, 0x73, 0xB0, 0xF7, 0xDA, 0x0E, 0x64, 0x52, 0xC8, 0x10, 0xF3, 0x2B,
        0x80, 0x90, 0x79, 0xE5, 0x62, 0xF8, 0xEA, 0xD2, 0x52, 0x2C, 0x6B, 0x7B,
    };
    unsigned char expanded_key[] = {
        ,
    };

    struct aes_context* ctx = aes_init_context(AES_192);
    CU_ASSERT_PTR_NOT_NULL_FATAL(ctx);

    for (int i = 0; i < 24; i++) {
        ctx->key->data[i] = key[i];
    }

    int result = aes_key_expansion(ctx);
    CU_ASSERT_EQUAL(result, 0);

    for (int i = 0; i < ctx->key_schedule->length; i++) {
        CU_ASSERT_EQUAL(expanded_key[i], ctx->key_schedule->data[i]);
    }

    aes_free_context(&ctx);
    CU_ASSERT_PTR_NULL(ctx);
}

void test_aes_key_expansion_AES_256() {
    unsigned char key[] = {
        0x60, 0x3D, 0xEB, 0x10, 0x15, 0xCA, 0x71, 0xBE, 0x2B, 0x73, 0xAE,
        0xF0, 0x85, 0x7D, 0x77, 0x81, 0x1F, 0x35, 0x2C, 0x07, 0x3B, 0x61,
        0x08, 0xD7, 0x2D, 0x98, 0x10, 0xA3, 0x09, 0x14, 0xDF, 0xF4,
    };
    unsigned char expanded_key[] = {
        ,
    };

    struct aes_context* ctx = aes_init_context(AES_256);
    CU_ASSERT_PTR_NOT_NULL_FATAL(ctx);

    for (int i = 0; i < 32; i++) {
        ctx->key->data[i] = key[i];
    }

    int result = aes_key_expansion(ctx);
    CU_ASSERT_EQUAL(result, 0);

    for (int i = 0; i < ctx->key_schedule->length; i++) {
        CU_ASSERT_EQUAL(expanded_key[i], ctx->key_schedule->data[i]);
    }

    aes_free_context(&ctx);
    CU_ASSERT_PTR_NULL(ctx);
}

void test_aes_shiftrows() {
    unsigned char state[] = {
        0x85, 0x82, 0x85, 0x8F, 0x6E, 0x19, 0xEA, 0xF6,
        0x61, 0x13, 0xBC, 0xC2, 0x3C, 0xE0, 0x26, 0xED,
    };

    unsigned char expected[] = {
        0x85, 0x19, 0xBC, 0xED, 0x6E, 0x13, 0x26, 0x8F,
        0x61, 0xE0, 0x85, 0xF6, 0x3C, 0x82, 0xEA, 0xC2,
    };

    aes_shiftrows(state);

    for (int i = 0; i < 16; i++)
        CU_ASSERT_EQUAL(expected[i], state[i]);
}

void test_aes_inv_shiftrows() {
    unsigned char state[] = {
        0x85, 0x82, 0x85, 0x8F, 0x6E, 0x19, 0xEA, 0xF6,
        0x61, 0x13, 0xBC, 0xC2, 0x3C, 0xE0, 0x26, 0xED,
    };

    unsigned char expected[] = {
        0x85, 0xE0, 0xBC, 0xF6, 0x6E, 0x82, 0x26, 0xC2,
        0x61, 0x19, 0x85, 0xED, 0x3C, 0x13, 0xEA, 0x8F,
    };

    aes_inv_shiftrows(state);

    for (int i = 0; i < 16; i++)
        CU_ASSERT_EQUAL(expected[i], state[i]);
}

void test_aes_mix_columns() {
    unsigned char state[] = {
        0x85, 0x19, 0xBC, 0xED, 0x6E, 0x13, 0x26, 0x8F,
        0x61, 0xE0, 0x85, 0xF6, 0x3C, 0x82, 0xEA, 0xC2,
    };

    unsigned char expected[] = {
        0x6B, 0x85, 0xD3, 0xF0, 0x40, 0xAD, 0xBB, 0x82,
        0x8A, 0xD8, 0x91, 0x31, 0xCD, 0xC4, 0x2C, 0xB3,
    };

    aes_mix_columns(state);

    for (int i = 0; i < 16; i++)
        CU_ASSERT_EQUAL(expected[i], state[i]);
}

void test_aes_encrypt_AES_128() {
    char input[] = {
        0x32, 0x43, 0xf6, 0xa8, 0x88, 0x5a, 0x30, 0x8d,
        0x31, 0x31, 0x98, 0xa2, 0xe0, 0x37, 0x07, 0x34,
    };

    char key[] = {
        0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
        0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c,
    };

    unsigned char expected[] = {
        0x39, 0x25, 0x84, 0x1D, 0x02, 0xDC, 0x09, 0xFB,
        0xDC, 0x11, 0x85, 0x97, 0x19, 0x6A, 0x0B, 0x32,
    };

    bytes_t* out = aes_encrypt(input, key, AES_128);
    CU_ASSERT_PTR_NOT_NULL_FATAL(out);

    for (int i = 0; i < 16; i++)
        CU_ASSERT_EQUAL_FATAL(expected[i], out->data[i]);

    bytes_free(&out);
}

void test_aes_decrypt_AES_128() {
    char input[] = {
        0x39, 0x25, 0x84, 0x1D, 0x02, 0xDC, 0x09, 0xFB,
        0xDC, 0x11, 0x85, 0x97, 0x19, 0x6A, 0x0B, 0x32,
    };

    char key[] = {
        0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
        0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c,
    };

    unsigned char expected[] = {
        0x32, 0x43, 0xf6, 0xa8, 0x88, 0x5a, 0x30, 0x8d,
        0x31, 0x31, 0x98, 0xa2, 0xe0, 0x37, 0x07, 0x34,
    };

    bytes_t* out = aes_decrypt(input, key, AES_128);
    CU_ASSERT_PTR_NOT_NULL_FATAL(out);

    for (int i = 0; i < 16; i++)
        CU_ASSERT_EQUAL_FATAL(expected[i], out->data[i]);

    bytes_free(&out);
}

void test_aes_encrypt_AES_192() {
    char input[] = {
        0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
        0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF,
    };

    char key[] = {
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B,
        0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
    };

    unsigned char expected[] = {
        0xDD, 0xA9, 0x7C, 0xA4, 0x86, 0x4C, 0xDF, 0xE0,
        0x6E, 0xAF, 0x70, 0xA0, 0xEC, 0x0D, 0x71, 0x91,
    };

    bytes_t* out = aes_encrypt(input, key, AES_192);
    CU_ASSERT_PTR_NOT_NULL_FATAL(out);

    for (int i = 0; i < 16; i++)
        CU_ASSERT_EQUAL_FATAL(expected[i], out->data[i]);

    bytes_free(&out);
};

void test_aes_decrypt_AES_192() {
    char input[] = {
        0xDD, 0xA9, 0x7C, 0xA4, 0x86, 0x4C, 0xDF, 0xE0,
        0x6E, 0xAF, 0x70, 0xA0, 0xEC, 0x0D, 0x71, 0x91,
    };

    char key[] = {
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B,
        0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
    };

    unsigned char expected[] = {
        0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
        0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF,
    };

    bytes_t* out = aes_decrypt(input, key, AES_192);
    CU_ASSERT_PTR_NOT_NULL_FATAL(out);

    for (int i = 0; i < 16; i++)
        CU_ASSERT_EQUAL_FATAL(expected[i], out->data[i]);

    bytes_free(&out);
};

void test_aes_encrypt_AES_256() {
    char input[] = {
        0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
        0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF,
    };

    char key[] = {
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A,
        0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
        0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F,
    };

    unsigned char expected[] = {
        0x8E, 0xA2, 0xB7, 0xCA, 0x51, 0x67, 0x45, 0xBF,
        0xEA, 0xFC, 0x49, 0x90, 0x4B, 0x49, 0x60, 0x89,
    };

    bytes_t* out = aes_encrypt(input, key, AES_256);
    CU_ASSERT_PTR_NOT_NULL_FATAL(out);

    for (int i = 0; i < 16; i++)
        CU_ASSERT_EQUAL_FATAL(expected[i], out->data[i]);

    bytes_free(&out);
};

void test_aes_decrypt_AES_256() {
    char input[] = {
        0x8E, 0xA2, 0xB7, 0xCA, 0x51, 0x67, 0x45, 0xBF,
        0xEA, 0xFC, 0x49, 0x90, 0x4B, 0x49, 0x60, 0x89,
    };

    char key[] = {
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A,
        0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
        0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F,
    };

    unsigned char expected[] = {
        0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
        0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF,
    };

    bytes_t* out = aes_decrypt(input, key, AES_256);
    CU_ASSERT_PTR_NOT_NULL_FATAL(out);

    for (int i = 0; i < 16; i++)
        CU_ASSERT_EQUAL_FATAL(expected[i], out->data[i]);

    bytes_free(&out);
};

CUNIT_CI_RUN("test_aes", CUNIT_CI_TEST(test_aes_key_expansion_AES_128),
             CUNIT_CI_TEST(test_aes_key_expansion_AES_192),
             CUNIT_CI_TEST(test_aes_key_expansion_AES_256),
             CUNIT_CI_TEST(test_aes_shiftrows),
             CUNIT_CI_TEST(test_aes_inv_shiftrows),
             CUNIT_CI_TEST(test_aes_mix_columns),
             CUNIT_CI_TEST(test_aes_encrypt_AES_128),
             CUNIT_CI_TEST(test_aes_decrypt_AES_128),
             CUNIT_CI_TEST(test_aes_encrypt_AES_192),
             CUNIT_CI_TEST(test_aes_decrypt_AES_192),
             CUNIT_CI_TEST(test_aes_encrypt_AES_256),
             CUNIT_CI_TEST(test_aes_decrypt_AES_256))
